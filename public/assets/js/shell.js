(()=>{
  ...
  if(header){
    header.outerHTML=`<header class="site-header"><div class="header-inner"><a class="brand-lock" href="index.html" aria-label="TAGRO ECHO home"><img class="tagro-mark" src="assets/brand/tagro-logo.png" alt="TAGRO" onerror="this.src='assets/brand/tagro-logo-actual.webp'"><span class="brand-cross" aria-hidden="true">×</span><img class="echo-mark" src="assets/brand/echo-logo-dark.svg" alt="ECHO"></a><nav class="desktop-primary-nav primary-nav" aria-label="Primary navigation">${nav.map(([k,u,l])=>`<a href="${u}" class="${page===k?'active':''}">${icons[k]}<span>${l}</span></a>`).join('')}</nav></div><nav class="class-nav" aria-label="Browse product classifications">${classes.map(([k,l])=>`<a href="${productLink(k)}">${l}</a>`).join('')}</nav></header><nav class="mobile-primary-nav primary-nav" aria-label="Primary navigation">${nav.map(([k,u,l])=>`<a href="${u}" class="${page===k?'active':''}">${icons[k]}<span>${l}</span></a>`).join('')}</nav>`;
  }
